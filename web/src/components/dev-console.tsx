...
export function DevConsole() {
  ...
  const poll = useCallback(async () => {
    const entries = await fetchLogs(sinceRef.current);
    if (entries.length > 0) {
      setLogs((prev) => {
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed bottom-0 right-0 z-40 flex w-full flex-col border-t border-border bg-surface-base sm:bottom-4 sm:right-4 sm:w-[520px] sm:rounded-xl sm:border"
          style={{ height: "320px" }}
        >
